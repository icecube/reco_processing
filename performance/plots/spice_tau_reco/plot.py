...
def compare( nufiles, outpath ):
    ...
    plot_mederes( data, fit_keys = ["loge_Taupede","loge_MillipedeTrun"], true_key = "loge_tru_neha",
                  labels=["Taupede", "MillipedeTrun"], outpath=outpath, bins = np.linspace(4,8,nbins) )
    plot_mederes( data, fit_keys = ["loge_Taupede","loge_MillipedeTrun"], true_key = "loge_tru_tianlu",
                  labels=["Taupede", "MillipedeTrun"], outpath=outpath, bins = np.linspace(4,8,nbins) )
    ...
    plot_mederes( data, fit_keys = ["loge_Monopod","loge_MillipedeTrun"], true_key = "loge_tru_neha",
                  labels=["Monopod", "MillipedeTrun"], outpath=outpath, bins = np.linspace(4,8,nbins) )
    plot_mederes( data, fit_keys = ["loge_Monopod","loge_MillipedeTrun"], true_key = "loge_tru_tianlu",
                  labels=["Monopod", "MillipedeTrun"], outpath=outpath, bins = np.linspace(4,8,nbins) )
...
# outpath = "/data/user/tvaneede/GlobalFit/reco_processing/performance/plots/spice_tau_reco/NuTau"
