...
for simulation_name in ["NuTau_midE"]:
# for simulation_name in ["NuE_midE","NuE_highE", "NuMu_midE", "NuMu_highE"]:
    ...
    for simulation_subfolder in simulation_subfolders:
        ...
        dag_path      = f"{dag_base_path}/{reco_input_version}/{dag_name}"
        log_dir       = f"{dag_path}/logs"
        backup_path   = f"{work_path}/backup_scripts/{reco_input_version}"
        ...
        os.system(f"cp run_pid.sub {dag_path}")

        # backup scripts
        os.system(f"cp {work_path}/run_pid.py {backup_path}")
        os.system(f"cp {work_path}/run_pid.sh {backup_path}")
        os.system(f"cp {work_path}/run_pid.sub {backup_path}")
        ...
        for i,INFILES in enumerate(infiles_list):
            ...
            outfile.write(f"JOB {JOBID} run_pid.sub\n")
