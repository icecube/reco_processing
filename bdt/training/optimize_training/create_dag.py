...
# version
version_name = "extra_train_sample_extra"

# fixed paths
dag_base_path = "/scratch/tvaneede/train_bdt"
work_path = "/data/user/tvaneede/GlobalFit/reco_processing/bdt/training/optimize_training"
...
os.system(f"cp train_bdt.sub {dag_path}")
...
for model_configs_name in model_configs_dict:
    for flux_model_name in flux_model_dict:
        for features_list_name in features_list_dict:
            ...
            outdir = f"{work_path}/output_extra_train_sample/{JOBID}"

            # create the dag job
            outfile.write(f"JOB {JOBID} train_bdt.sub\n")
