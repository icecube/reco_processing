...
if __name__ == "__main__":
    ...
    contour_dir = f"{output_dir}/contours"
    os.system(f"mkdir -p {output_dir}")
    os.system(f"mkdir -p {contour_dir}")

    config_template = f"{main_dir}/config_pyff_bins.yaml"
    ...
    df_input["bdt_product"] = df_input["bdt_scores1"]*df_input["bdt_scores2"]
    ...
    for i in range(len(bdt_cuts)):
        for j in range(len(bdt_cuts)):
            ...
            update_file(f"{this_path}/{config_input}", input_parameters=bins_settings[args.bins])
            
            # update the lower threshold bdt score if necessary
            update_file(f"{this_path}/{config_input}", input_parameters={"<BDT1_THRESHOLD>" : f"{cut_bdt1}","<BDT2_THRESHOLD>" : f"{cut_bdt2}","<BDTPRODUCT_THRESHOLD>" : f"{cut_bdt1*cut_bdt2:.3f}"})
            ...
            # if False:
                ...
                contour_size,x,y = plot_flavor_triangle(ftau,fe,dllhs, outpath=f"{contour_dir}/bdt1-{cut_bdt1}_bdt2-{cut_bdt2}.pdf")
                df_contour = pd.DataFrame({"x": x,"y": y,})
                df_contour.to_parquet(f"{contour_dir}/bdt1-{cut_bdt1}_bdt2-{cut_bdt2}.parquet")
            else:
