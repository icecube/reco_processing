...
# version
version_name = "optimize_binning_900steps_100fits_complete"
...
os.system(f"cp optimize_cuts_bins.sub {dag_path}")
...
for model_configs_name in ["simpletopology"]:
    for flux_model_name in ["hese"]:
        for features_list_name in features_list_dict:
            for bins_settings_name in ["13logE_10bdtprod"]:

                JOBID = f"mcd-{model_configs_name}_flux-{flux_model_name}_feat-{features_list_name}_bins-{bins_settings_name}"

                outdir = f"{work_path}/output/{version_name}/{JOBID}"

                # create the dag job
                outfile.write(f"JOB {JOBID} optimize_cuts_bins.sub\n")
                ...
                outfile.write(f'VARS {JOBID} BINS="{bins_settings_name}"\n')
