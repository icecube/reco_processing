...
work_path = "/data/user/tvaneede/GlobalFit/reco_processing/muons/hese/filter"

submit_jobs = True # actually submit the dag jobs

# fixed dag paths
dag_name = f"filter_dag_muongun_{filter_version}"
...
for simulation_name in ["MuonGun_lowE", "MuonGun_midE", "MuonGun_highE"]:
    ...
    level2_path = simulation_datasets[simulation_name]['level2_path']
    ...
    for simulation_subfolder in simulation_subfolders:
        ...
        # fixed paths
        filter_in_path = f"{level2_path}/{simulation_dataset}/{simulation_subfolder}"

        filter_out_path = f"{work_path}/output/{filter_version}/{simulation_dataset}"
        filter_out_file = f"{filter_out_path}/HESE_{simulation_dataset}_{simulation_subfolder}.i3.zst"
        ...
        outfile.write(f'VARS {JOBID} Outputfile="{filter_out_file}"\n')
